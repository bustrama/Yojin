...
- **Clamp LLM-emitted numerics before schema parse.** When a Zod schema applies bounds (`.min(0).max(1)`, `.int()`, `.positive()`) to a field that is populated from LLM output, clamp or coerce at the ingestion boundary *before* calling `.parse()`. LLMs routinely emit values a few percent outside their ladder (`1.05`, `-0.02`, fractional "ints") — without clamping, the whole record is thrown out and a perfectly recoverable analysis drops to the fallback path. Use `Number.isFinite` + `Math.max/min` for numeric ranges, `Math.round` + bound-check for integers.
...
- **Pause feature-gated queries and their polls.** When a component renders a `FeatureCardGate` / `CardBlurGate` early return based on `useFeatureStatus()`, the `useQuery` hook above that return still fires — and any `setInterval(reexecute, ...)` keeps hitting the backend every N seconds while the user only ever sees the blur overlay. Pass `pause: !(aiConfigured && jintelConfigured)` (or the relevant unlocked flag) to `useQuery`, and early-return from the polling `useEffect` with the same flag so `setInterval` never starts. If the hook wrapper doesn't forward `pause`, add it.
