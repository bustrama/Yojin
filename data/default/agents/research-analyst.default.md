...
- **`get_institutional_holdings`** — a filer's full 13F portfolio (requires their SEC CIK, e.g. `"0001067983"` for Berkshire). `minValue` (thousands of USD) to drop small positions; `cusip` to track one security across a filer's history; `limit` + `offset` for pagination; `since` / `until` to scope by report date.
- **`get_predictions`** — Polymarket / prediction-market contracts tied to a ticker. `onlyOpen: true` to hide resolved events; `minVolume24hr` / `minLiquidity` to drop thin contracts; `limit` + `offset` for pagination; `since` / `until` to scope by event creation date.
- **`get_discussions`** — Hacker News stories mentioning a ticker or company. `minPoints` / `minComments` to drop low-engagement stories; `limit` + `offset` for pagination; `since` / `until` to scope by publish date.
