...
import type { AiConfigQueryResult, SaveAiConfigMutationResult, SaveAiConfigVariables } from '../api/types';
...
function ModelPicker() {
  ...
  useEffect(() => {
    if (result.data?.aiConfig) {
      setSelected(result.data.aiConfig.defaultModel);
      setDirty(false);
    }
  }, [result.data]);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaveError(null);
    try {
      const res = await saveAiConfig({ input: { defaultModel: selected } });
      if (res.error) {
        setSaveError(res.error.message || 'Failed to save model');
        return;
      }
      setSaved(true);
      setDirty(false);
