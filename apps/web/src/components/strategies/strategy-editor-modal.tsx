...
export default function StrategyEditorModal({ open, onClose, strategy: editStrategy }: StrategyEditorModalProps) {
  ...
  const initial = editStrategy
    ? {
        name: editStrategy.name,
        description: editStrategy.description,
        category: editStrategy.category,
        style: editStrategy.style,
        requires: [...editStrategy.requires],
        content: editStrategy.content,
        triggers: editStrategy.triggers.map((t) => ({
          type: t.type,
          description: t.description,
          params: t.params ?? '',
        })),
        tickers: editStrategy.tickers.join(', '),
        maxPositionSize: editStrategy.maxPositionSize != null ? String(editStrategy.maxPositionSize) : '',
      }
    : defaults;
  ...
  async function handleSave() {
    ...
    try {
      ...
      const result =
        isEdit && editStrategy ? await updateStrategy({ id: editStrategy.id, input }) : await createStrategy({ input });
