      ...
      // Stage 2: Strategist — 1 iteration to batch all tool calls, 1 to emit final text
      {
        ...
        buildMessage: (prev) => {
          ...
          prompt +=
            ...
            `Fields:\n` +
            `- positions[]: symbol, name, rating (VERY_BULLISH/BULLISH/NEUTRAL/BEARISH/VERY_BEARISH), conviction, thesis (2-3 sentences explaining WHY — cite macro/sector/geopolitical drivers. Lead with the STORY, not the datapoint: NEVER start (or compose the thesis as) a raw indicator reading like "MFI 75.", "RSI 80", "Price 108.45" — those belong in signals, the thesis must be prose narrative with at least two meaningful alphabetic tokens per sentence. Cross-position links ONLY with concrete evidence (shared supply chain, same macro driver, correlated sector): cite the evidence and estimate impact magnitude. Do NOT invent connections), keySignals[] (top 2 per position), risks[] (1-2 — ticker-specific risks belong HERE, not in portfolio.topRisks; write full phrases, not bare indicator values), opportunities[] (1-2 — ticker-specific opportunities belong HERE, not in portfolio.topOpportunities; write full phrases, not bare indicator values), memoryContext: null, priceTarget: null\n` +
