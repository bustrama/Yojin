...
export const SELF_INVALIDATING_RE =
  /\blacks (?:independent )?verification\b|(?<!(?:denied|refuted|dismissed|rejected) (?:an? )?)\bunverified claim\b|\bremains? speculative\b|\brel(?:y|ies) on anecdotal\b|\banecdotal (?:customer |user |individual )?(?:commentary|observation|evidence|report|claim|feedback|account)\b|\bnot (?:verified|confirmed) (?:market )?data\b|\bno (?:independent )?confirmation from (?:the )?(?:company|management)\b|\bcannot be (?:verified|substantiated|corroborated)\b|\bself[- ]reported (?:and )?unverified\b|\bno (?:supporting )?evidence (?:to (?:support|back|substantiate)|beyond|for (?:the |this ))\b|\bbased on (?:a single|one) (?:user'?s?|person'?s?) (?:observation|claim|report)\b|\[(?:removed|deleted)\]|\b(?:post|source) (?:(?:is|has been|was) )?(?:unavailable|removed|deleted)\b|\b(?:claim|thesis|premise) (?:is |remains? )?contested\b|\bmaking the (?:claim|thesis) contested\b|\bincludes? pushback\b|\bcontested (?:claim|thesis|premise|conclusion)\b/i;
